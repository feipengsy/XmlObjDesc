...
#--------------------------------------------------------------------------------
  def reset(self, package, godClass):
    ...
    self.genOStream = 0
    self.genFillStream = 0
    if 'in' in godClass : self.isEventClass = 1
...
#--------------------------------------------------------------------------------
  def genEDMBook(self, godClass, scopeName=''):
    ...
    if 'path' not in classAtt or 'priority' not in classAtt:
      return s
    if 'SmartRelation' not in godClass:
      return s
...
#--------------------------------------------------------------------------------
  def genConstructors(self,godClass,clname=''):
    ...
    if 'constructor' in godClass:                                         # are there any constrs defined
    ...
    if not (hasDefaultConstructor or clname):                                   # no constructors defined lets
      ...
      indent = ' ' * (len(s2) + 3)
      s += s2
      if 'attribute' in godClass :                                        # if there are attributes
    ...
    if 'bitfield' in att:
      ...
      for bf in att['bitfield']:
        ...
        if bfAtt['length'].isdigit():
          ...
          mask = 0
          for j in range(offset,offset+lInt) : mask += (1<<j)
          offset += lInt

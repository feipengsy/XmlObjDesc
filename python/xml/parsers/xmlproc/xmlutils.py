...
import string,re,urllib,os,sys,types
import urllib.parse
...
try:
    ...
    using_unicode = 1
    xml_chr = chr
    BOM = b"\xfe\xff".decode("utf-16-be")
except ImportError:
    def mkconverter(parser,src,dest):
        if dest == None:
            dest = "utf-8"
...
class EntityParser:
    ...
    def set_error_language(self,language):
        ...
        self.errors=errors.get_error_list(str.lower(language))
        self.err_lang=str.lower(language) # only set if supported
    ...
    def feed(self, new_data, decoded = 0):
        ...
        self.data=str.replace(self.data+new_data,
                                 "\015\012","\012")
    ...
    def scan_to(self,target):
        ...
        new_pos=str.find(self.data,target,self.pos)
        if new_pos==-1:
            raise OutOfDataException()
    ...
    def update_pos(self):
        ...
        breaks=str.count(self.data,"\n",self.last_upd_pos,self.pos)
        self.last_upd_pos=self.pos

        if breaks>0:
            self.line=self.line+breaks
            self.last_break=str.rfind(self.data,"\n",0,self.pos)
...
def join_sysids_general(base, url):
    ...
    if urllib.parse.urlparse(url)[0] != "":
        return url
    elif urllib.parse.urlparse(base)[0] == "":
        if urllib.parse.urlparse(url)[0] == "":
...
def join_sysids_win32(base, url):
    ...
    if urllib.parse.urlparse(url)[0] != "":
        return url
    elif len(urllib.parse.urlparse(base)[0])<2: # Handles drive identifiers correctly
...
if using_unicode:
    for k in predef_ents.keys():
        predef_ents[k]=str(predef_ents[k])

# Translation tables

_ws_trans=str.maketrans("\r\t\n","   ")  # Whitespace normalization
if using_unicode:
    _ws_dict = {}
    for c in "\r\t\n":_ws_dict[ord(c)] = ord(' ')

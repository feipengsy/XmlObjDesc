...
#import urllib,os,anydbm,string,cPickle,time
import urllib,os,string,time

from xml.parsers.xmlproc.xmlproc import *
from xml.parsers.xmlproc.xmldtd import *
from xml.parsers.xmlproc.xmlapp import *
...
class ValidatingApp(Application):
    ...
    def handle_end_tag(self,name):
        ...
        if self.cur_elem != None and \
            ...
            self.parser.report_error(2004, (name, ", ".join(valid)))
	...
	# Check the values of the present attributes
        for attr in attrs.keys():
            ...
            if decl.type!="CDATA":
                attrs[attr]=' '.join(str.split(attrs[attr]))
            ...
            if decl.type=="ID":
                ...
                for idref in str.split(attrs[attr]):
                    self.idrefs.append((self.locator.get_line(),
                                        self.locator.get_column(),
                                        idref))
                ...
                for ent_ref in str.split(attrs[attr]):
                    self.__validate_attr_entref(ent_ref)

        # Check for missing required attributes
        for attr in element.get_attr_list():
            decl=element.get_attr(attr)
            if decl.decl=="#REQUIRED" and not (attr in attrs):

...
class WFCDTD(DTDConsumer):
    ...
    def new_attribute(self,elem,attr,a_type,a_decl,a_def):
        ...
        if not ( elem in self.elems.keys()):
    ...
    def new_parameter_entity(self,name,val):
        if name in self.param_ents.keys():
            ## FIXME: May warn
            return # Keep first decl
    ...
    def new_external_entity(self,ent_name,pubid,sysid,ndata):
        if ent_name in self.gen_ents.keys():
            ## FIXME: May warn
            return # Keep first decl

        if ndata != None and hasattr(self, "notations"):
            if not (ndata in self.notations.keys()):
...
class CompleteDTD(WFCDTD):
    ...
    def new_element_type(self,elem_name,elem_cont):
        if elem_name in self.elems.keys():
        ...
        if elem_name in self.attlists.keys():
...
class ElementType:
    ...
    def next_state_skip(self, state, elem_name):
        ...
        for skipped in arcs.keys():
            if elem_name in self.content_model[arcs[skipped]].keys():
...
class Attribute:
    ...
    def __init__(self,name,attrtype,decl,default,parser):
        ...
        if default!=None and self.type!="CDATA":
            self.default=' '.join(str.split(default))
        else:
...
class ContentModel:
    ...
    def add_contents(self,builder,loop=0):
        ...
        if not isinstance(self.contents[0],str):
            if loop:
                builder.remember_state()
...
#             parser.report_error(1008)
        ...
        if len(destlist)==1 and (destlist[0] in closure_hash.keys()):
        ...
        if not ( new_state in new_states.keys()):
...
def print_states(states,stop=0):
    assert not ("start" in states.keys() or "final" in states.keys())
...
def make_model(cmhash,content_model,err):
    ...
    cm=str(content_model)
    if cm in cmhash.keys():
        return cmhash[cm]
    else:
